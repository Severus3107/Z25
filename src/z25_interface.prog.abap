...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  METHOD lif_partner~get_partner_info.
    rt_info = VALUE #( ( attribute = 'TYPE' value = 'CARRIER')
                       ( attribute = 'VEHICLES' value = | { lines( mt_planes ) } |  ) ).
  ENDMETHOD.
...
DATA: go_airplane       TYPE REF TO lcl_airplane,
      ...
      gt_partnertab     TYPE STANDARD TABLE OF REF TO lif_partner,
...
* Create airline and car rental
  ...
  gt_partnertab = VALUE #( ( go_carrier ) ( go_rental ) ).
...
* Create some vehicles.
  TRY.
    ...
    message 'vehicle not found' type 'I'.
  ENDTRY.
  ...
  LOOP AT gt_partnertab INTO gv_partner.
    gt_single_partner = gv_partner->get_partner_info( ).
